<template>
  <div class="report">
    <h2 class="page-title">51项补贴项目发放表</h2>
    ...
    <el-card shadow="never" class="filter">
      <el-form inline @submit.prevent>
        <el-form-item label="项目名称：">
          <el-select v-model="q.code" filterable clearable placeholder=" " style="width:360px">
            <el-option v-for="it in items" :key="it.code" :label="`${it.code}-${it.itemName}`" :value="it.code" />
          </el-select>
        </el-form-item>
...
const items = ref([]); const counties = ref([])
const q = reactive({ code: null, countyCode: null })
const rows = ref([]); const loading = ref(false)

onMounted(async () => {
  const [is, cs] = await Promise.all([reportApi.std51Items(), reportApi.counties()])
  items.value = is || []; counties.value = cs || []
  reload()
})
...
async function reload() {
  loading.value = true
  try {
    rows.value = (await reportApi.std51({
      code: q.code || undefined, countyCode: q.countyCode || undefined
    })) || []
...
function doExport() {
  ...
  a.href = url; a.download = '51项补贴项目发放表.csv'; document.body.appendChild(a); a.click()
