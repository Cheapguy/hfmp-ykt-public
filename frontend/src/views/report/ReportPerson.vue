<template>
  <div class="report">
    <h2 class="page-title">乡镇发放情况个人查询</h2>
    ...
    <el-card shadow="never" class="filter">
      <div class="filter-grid">
        ...
        <template v-if="expanded">
          <div class="fi"><label>户主姓名：</label><el-input v-model="q.holderName" clearable @keyup.enter="reload" /></div>
          <div class="fi"><label>户主身份证号：</label><el-input v-model="q.holderIdCard" clearable @keyup.enter="reload" /></div>
          <div class="fi"><label>享受人姓名：</label><el-input v-model="q.beneficiaryName" clearable @keyup.enter="reload" /></div>
          <div class="fi"><label>享受人身份证：</label><el-input v-model="q.beneficiaryIdCard" clearable @keyup.enter="reload" /></div>
    ...
    <el-card shadow="never">
      <div class="sum-bar">合计发放金额 <b>{{ money(totalAmount) }}</b> 元 · 共 {{ total }} 人</div>
      <el-table v-loading="loading" :data="rows" border stripe size="default">
        ...
        <el-table-column prop="projectName" label="补贴发放项目" width="140" show-overflow-tooltip />
        <el-table-column prop="batchName" label="补贴发放批次" min-width="220" show-overflow-tooltip />
        ...
        <el-table-column prop="grantTime" label="发放时间" width="160">
          <template #default="{ row }">{{ fmtTime(row.grantTime) }}</template>
        </el-table-column>
      </el-table>
      <div class="pager">
...
const q = reactive({ projectId: null, batchId: null, agency: null, village: null, holderName: '', holderIdCard: '', beneficiaryName: '', beneficiaryIdCard: '' })
...
async function reload() {
  loading.value = true
  try {
    const res = await reportApi.person({
      pageNum: page.pageNum, pageSize: page.pageSize,
      projectId: q.projectId || undefined, batchId: q.batchId || undefined,
      holderName: q.holderName || undefined, holderIdCard: q.holderIdCard || undefined,
      beneficiaryName: q.beneficiaryName || undefined, beneficiaryIdCard: q.beneficiaryIdCard || undefined
    }) || {}
...
function doExport() {
  ...
  const head = ['补贴发放项目', '补贴发放批次', '户主身份证', '户主姓名', '享受人身份证', '享受人姓名', '乡镇', '村组', '发放金额', '发放时间']
  const lines = rows.value.map(r => [r.projectName, r.batchName, r.holderIdCard, r.holderName, r.beneficiaryIdCard, r.beneficiaryName, r.townName, r.villageName, r.amount, fmtTime(r.grantTime)]
    .map(v => `"${v == null ? '' : String(v).replace(/"/g, '""')}"`).join(','))
  const csv = '﻿' + [head.join(','), ...lines].join('\r\n')
  const blob = new Blob([csv], { type: 'text/csv;charset=utf-8' })
  const url = URL.createObjectURL(blob); const a = document.createElement('a')
  a.href = url; a.download = '乡镇发放情况个人查询.csv'; document.body.appendChild(a); a.click()
...
.filter-actions { display: flex; gap: 10px; margin-top: 16px; padding-top: 14px; border-top: 1px solid var(--el-border-color-light); }
